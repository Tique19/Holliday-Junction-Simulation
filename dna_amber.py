...
class Design:
   ...
   def solvate_amber(self,top_file,temp_dir,wfile):
      #Creat prmtop and inpcrd files for the neutralized DNA origami
      with working_directory(temp_dir):
         self.export_amber_solvate('make_amber_solvate',top_file,wfile)
   ...
   def export_amber_solvate(self, filename, top_file, wfile):
      f = open(filename + ".script", 'w')
   ...
   def autoionize(self,temp_dir,top_file,wfile):
      # add extra ions
      with working_directory(temp_dir):
...
def convert_origami():
   ...
   # Topology file
   top_file = "leaprc.DNA.OL15"
   # TIP3PBOX source file
   wfile = "leaprc.water.tip3p"
   ...
   d.solvate_amber(top_file,temp_dir,wfile)
   d.autoionize(temp_dir,top_file,wfile)
...
for key in keys:
   if not search("AMBERHOME", key):
      os.environ["AMBERHOME"]="/Users/tylerking/amber18"
